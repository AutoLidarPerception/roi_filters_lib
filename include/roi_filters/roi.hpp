...
namespace roi {
    ...
    static void elevationBandPass(float z_limit_min, float z_limit_max,
     ...
     *      in order to filter-out noise in voxels where there is not enough evidence for occupancy.
     ...
     */
    template <typename PointT>
    static void voxelGridFilter(float voxel_size, int min_point_number_per_voxel,
                                typename pcl::PointCloud<PointT>::Ptr cloud)
    {
        if (cloud->size()) {
            typename pcl::PointCloud<PointT>::Ptr cloud_in(new pcl::PointCloud<PointT>);
    ...
    template <typename PointT>
    static void approxVoxelGridFilter(float voxel_size, int min_point_number_per_voxel,
                                      typename pcl::PointCloud<PointT>::Ptr cloud)
    {
        if (cloud->size()) {
            typename pcl::PointCloud<PointT>::Ptr cloud_in(new pcl::PointCloud<PointT>);
    ...
    /*
     * @brief Cylinder(middle center[x,y,z], radius_m, height_above_m, height_below_m) Filter
     * @result cloud after filter
     */
    template <typename PointT>
    static void cylinderROIFilter(float radius_min, float radius_max,
     ...
     */
    template <typename PointT>
    static void squareROIFilter(float radius_min, float radius_max,
                                float z_limit_min, float z_limit_max,
                                typename pcl::PointCloud<PointT>::Ptr cloud)
    {
        if (cloud->size()) {
                               ...
                               typename pcl::PointCloud<PointT>::Ptr cloud)
    {
        ...
        const float roi_z_limit_min = (-1.0) * (params.roi_lidar_height_m + params.roi_height_below_m);
        const float roi_z_limit_max = params.roi_height_above_m - params.roi_lidar_height_m;
        ...
        if (params.type == "Cylinder") {
            ...
            squareROIFilter<PointI>(roi_radius_min, roi_radius_max,
                                    roi_z_limit_min, roi_z_limit_max, cloud);
        }
        else {
            cylinderROIFilter<PointI>(roi_radius_min, roi_radius_max,
     ...
     */
    template <typename PointT>
    static void bevROIFilter(float x_forward, float x_back,
                             float half_width,
                             typename pcl::PointCloud<PointT>::Ptr cloud)
    {
        if (cloud->size()) {
                                    ...
                                    std::vector<ObjectPtr>* objects_filtered)
    {
        ...
        (*objects_filtered).insert((*objects_filtered).end(), objects.begin(), objects.end());
    }
}
